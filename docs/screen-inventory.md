...
| --- | ---: | --- |
| UI routes implemented | 29 | Includes dynamic section routes, Ask Pal, login, My Classes, Class Overview, Attendance, Attendance Register, Lesson Plan Detail, Assessment Detail, Gradebook Sheet, Reports, Class Report, Approvals, and Approval Detail. |
| Mock API routes | 5 | JSON route handlers under `src/app/api`. |
| Integrated demo slices | 16 | `/classes`, `/classes/[classId]`, `/attendance`, `/attendance/[classId]`, `/lesson-planner`, `/lesson-planner/new`, `/lesson-planner/[lessonPlanId]`, `/assessments`, `/assessments/new`, `/assessments/[assessmentId]`, `/gradebook`, `/gradebook/[assessmentId]`, `/reports`, `/reports/[classId]`, `/approvals`, and `/approvals/[approvalId]` use auth, tenant, permission, service, DTO, and adapter layers backed by in-memory seed data. |
| Partially integrated | 24 | Prototype API handlers plus `/api/v1` demo-backed handlers. |
| Mock-only | 8 | Remaining teacher workspace screens depend on mock data/config. |
