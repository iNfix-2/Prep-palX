...
import type { GradebookCardView } from "@/lib/features/gradebook/types";
...
export function GradebookCard({ gradebook }: { gradebook: GradebookCardView }) {
  return (
    <Link href={gradebook.href} className="block h-full">
      <Card className="flex h-full flex-col gap-5 p-5 transition-colors hover:border-primary">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-body-md text-muted">{gradebook.subjectLabel}</p>
          </div>
          <span
            className={cn(
              "shrink-0 rounded-full border px-2.5 py-1 text-label-sm font-bold",
              statusClasses[gradebook.statusTone],
            )}
          >
        ...
        <div className="grid grid-cols-2 gap-3 text-body-md">
          <Meta icon="calendar_today" label={gradebook.dueLabel} />
          <Meta icon="grading" label={gradebook.marksLabel} />
          <Meta icon="groups" label={gradebook.learnerLabel} />
          <Meta icon="task_alt" label={gradebook.scoredLabel} />
        </div>

        <div className="rounded-lg border border-surface-border bg-background p-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-body-md font-bold text-foreground">{gradebook.averageLabel}</p>
            <p className="text-body-md text-muted">{gradebook.missingLabel}</p>
          </div>
        </div>
