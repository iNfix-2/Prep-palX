...
import type { CreateQuestionDto } from "@/lib/features/question-bank/types";
...
export async function POST(request: NextRequest) {
  const body = (await request.json().catch(() => null)) as CreateQuestionDto | null;

  if (!body) {
    return apiError("VALIDATION_ERROR", "Request body must be valid JSON.", 400);
  }

  const result = createQuestion(getRequestAuthContext(request), body);
