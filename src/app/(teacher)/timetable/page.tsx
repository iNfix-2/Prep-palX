import Link from "next/link";
import { TimetableEventCard } from "@/components/timetable/timetable-event-card";
import {
  EmptyState,
  ForbiddenState,
  UnauthorizedState,
} from "@/components/states/data-states";
import { Card } from "@/components/ui/card";
import { Icon } from "@/components/ui/icon";
import { toTimetablePageView } from "@/lib/features/timetable/adapters";
import { getPageAuthContext } from "@/lib/server/auth-context";
import { listTeacherTimetable } from "@/lib/server/timetable-service";

export default async function TimetablePage({
  searchParams,
}: {
  searchParams: Promise<{ date?: string }>;
}) {
  const { date } = await searchParams;
  const result = listTeacherTimetable(await getPageAuthContext(), date);

  if (!result.ok) {
    if (result.status === 401) {
      return <UnauthorizedState />;
    }

    return <ForbiddenState message={result.message} />;
  }

  const view = toTimetablePageView(result.data);

  if (view.events.length === 0) {
    return (
      <EmptyState
        title="No timetable events"
        message="There are no accessible events for the selected school day."
        action={{ label: "Open classes", href: "/classes", icon: "groups" }}
      />
    );
  }

  return (
    <div className="mx-auto w-full max-w-(--container-max) space-y-6">
      <header className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div className="max-w-3xl">
          <p className="text-label-md font-bold uppercase tracking-wider text-muted">
            {view.workspaceName}
          </p>
          <h1 className="mt-2 font-display text-display-lg font-extrabold text-foreground">
            Timetable
          </h1>
          <p className="mt-2 text-body-lg text-muted">
            {view.selectedDateLabel} schedule with class-linked preparation context.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Link
            href="/timetable"
            className="inline-flex h-10 items-center justify-center gap-2 rounded-md bg-primary px-4 text-body-md font-semibold text-on-primary transition-colors hover:bg-primary-hover"
          >
            <Icon name="calendar_today" className="text-[18px]" />
            Today
          </Link>
          <Link
            href="/academic-calendar"
            className="inline-flex h-10 items-center justify-center gap-2 rounded-md border border-surface-border bg-surface px-4 text-body-md font-semibold text-foreground transition-colors hover:bg-background"
          >
            <Icon name="calendar_month" className="text-[18px]" />
            Calendar
          </Link>
          {view.canUseAi && (
            <Link
              href="/ask-pal"
              className="inline-flex h-10 items-center justify-center gap-2 rounded-md bg-ai px-4 text-body-md font-semibold text-on-ai transition-colors hover:bg-ai-hover"
            >
              <Icon name="smart_toy" className="text-[18px]" />
              Prep with Pal
            </Link>
          )}
        </div>
      </header>

      <section className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <MetricCard icon="calendar_view_day" label="Events" value={String(view.totalEvents)} />
        <MetricCard icon="auto_stories" label="Teaching" value={String(view.teachingBlockCount)} />
        <MetricCard
          icon="priority_high"
          label="Prep flags"
          value={String(view.preparationIssueCount)}
        />
        <MetricCard icon="location_on" label="Locations" value={String(view.locationCount)} />
      </section>

      <div className="grid grid-cols-12 gap-6">
        <section className="col-span-12 space-y-4 xl:col-span-8">
          {view.events.map((event) => (
            <TimetableEventCard key={event.id} event={event} />
          ))}
        </section>

        <aside className="col-span-12 space-y-6 xl:col-span-4">
          <Card>
            <div className="flex items-center gap-2">
              <Icon name="schedule" className="text-primary" />
              <h2 className="font-display text-headline-sm text-foreground">Day Focus</h2>
            </div>
            <p className="mt-3 text-body-md text-muted">
              Timetable blocks are filtered to your active workspace and assigned classes unless
              your membership can manage classes.
            </p>
          </Card>

          <Card>
            <div className="flex items-center gap-2">
              <Icon name="task_alt" className="text-primary" />
              <h2 className="font-display text-headline-sm text-foreground">Preparation</h2>
            </div>
            <p className="mt-3 font-display text-[28px] font-extrabold leading-none text-foreground">
              {view.preparationIssueCount}
            </p>
            <p className="mt-2 text-body-md text-muted">
              Events need a final preparation check before the school day is clear.
            </p>
          </Card>

          <Card className="border-ai/20 bg-ai text-on-ai">
            <div className="flex items-center gap-2">
              <Icon name="smart_toy" filled />
              <h2 className="text-body-lg font-bold">Pal Suggestion</h2>
            </div>
            <p className="mt-3 text-body-md text-white/90">
              Convert the remaining prep flags into a short before-class checklist.
            </p>
          </Card>
        </aside>
      </div>
    </div>
  );
}

function MetricCard({ icon, label, value }: { icon: string; label: string; value: string }) {
  return (
    <Card className="p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-label-md font-bold uppercase tracking-wider text-muted">{label}</p>
          <p className="mt-2 font-display text-[28px] font-extrabold leading-none text-foreground">
            {value}
          </p>
        </div>
        <div className="flex h-11 w-11 items-center justify-center rounded-lg border border-primary/20 bg-primary/10 text-primary">
          <Icon name={icon} />
        </div>
      </div>
    </Card>
  );
}
